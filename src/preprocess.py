...
def load_data():
    ...
    # chargement des 4 fichiers FakeNewsNet originaux
    for key, filename in config.RAW_FILES.items():
        ...
        frames.append(df[['title', 'text', 'label', 'source']])

    # chargement du dataset supplémentaire s'il existe
    for key, filename in config.EXTRA_FILES.items():
        path = os.path.join(config.DATA_RAW_DIR, filename)
        if not os.path.exists(path):
            continue
        df = pd.read_csv(path)
        df['label']  = config.EXTRA_LABELS[key]
        df['source'] = 'extra'
        frames.append(df[['title', 'text', 'label', 'source']])
    ...
    df_all = df_all.dropna(subset=['text', 'title'])

    print(f'Dataset total : {len(df_all)} articles')
